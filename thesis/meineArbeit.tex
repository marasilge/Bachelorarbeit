...
The complete Lean code accompanying this thesis can be found at (ToDo) % ToDo: Link zum Repository
...
\end{proof}
Combining the two lemmas gives the \emph{retraction criterion}, which is the tool we use in several of the following proofs.In the lecture notes, the criterion is stated only for closed subspaces $A \subseteq X$. Hatcher \cite{HAT01} states it without this assumption, but for the implication from the existence of a retraction to the HEP he gives a proof under the additional hypothesis that $A$ is closed, referring to the appendix for the general case, where a detailed proof following an argument of \cite{STM} is given. Our formulation follows the same pattern as \cref{lem:HEPextension}. We impose closedness only on the implication that actually needs it. \\
...
\begin{proposition}[Retraction Criterion, cf.\ {\cite[Bemerkung~4]{WAL}}]  \label{prop:retractioncriterion}
...
Since the definition of CW-complexes in mathlib uses $n$-dimenstional cubes rather than closed balls as the domain of the caracteristic maps, we restate the HEP for cubes relative to their boundary. \\
...
-- ToDo: Question: Das folgende Lemma hat wieder keine Quelle. Das ist okay, oder? %(eventuell auch in https://hbz-ulb.primo.exlibrisgroup.com/discovery/fulldisplay?docid=cdi_springer_books_10_1007_BFb0083031&context=PC&vid=49HBZ_ULB:DEFAULT&lang=de&search_scope=MyInst_and_CI&adaptor=Primo%20Central&tab=Everything&query=any,contains,tom%20Dieck,%20Algebraic%20Topology
)
...
This map will be obtained from the universal property of the quotient. To use it we need to know that $X_m \times I$ carries the quotient topology, and for this it suffices to know that $X_m$ does, by the following theorem: (ToDo: Quelle in der MNL anschauen und einfügen - ist bestellt) %https://hbz-ulb.primo.exlibrisgroup.com/discovery/fulldisplay?docid=alma991009987839706467&context=L&vid=49HBZ_ULB:DEFAULT&lang=de&search_scope=MyInst_and_CI&adaptor=Local%20Search%20Engine&isFrbr=true&tab=Everything&query=any,contains,general%20Topology,%20Ryszard%20Engelking&sortby=date_d&facet=frbrgroupid,include,9035791847587913213&offset=0
...
We introduce those aspects of mathlib's formalisation of CW-complexes that are relevant for us. The reference for this section is the documentation of \lean{Topology.RelCWComplex}.   (ToDo: cite/ link %https://leanprover-community.github.io/mathlib4_docs/Mathlib/Topology/CWComplex/Classical/Basic.html#Topology.RelCWComplex.skeletonLT
...
\end{minted}
Here the complex \lean{X} and its base space \lean{C} are both sets of the ambient type \lean{Y}. Mathlib's definition does not assume the ambient space to be Hausdorff. Nevertheless, for many statements about complexes we have to add the instance \lean{[T2Space Y]}. \\
...
Already in the mathematical part, the definition of the homotopy extension propert was long and technical and its formalisation is no shorter. Throughout the formalisation project we use the follwoing abbreviations, and for this section we fix the variables below. 
\begin{minted}{lean4}
universe u
...
We can now turn to the definition itself.

\begin{minted}{lean4}
def agreeOn (f : X → Y) (H : X × ℝ → Y) (A : Set X) : Prop := ∀ (a : X), a ∈ A → f a = H (a,0)

structure HomotopyExtension (H' : X × ℝ → Y) (f : X → Y) (H : X × ℝ → Y) (A : Set X) (rangeH' : Set Y) : Prop  where
...
def HEP (X : Type u) [TopologicalSpace X] (A : Set X) : Prop :=
  ∀ (Y : Type u) [TopologicalSpace Y] (rangeH' : Set Y), ∀ (f : X → Y), ∀ (H : X × ℝ → Y), Continuous f → Set.range f ⊆ rangeH' → ContinuousOn H (smalcyl A) → (smalcyl A).MapsTo H rangeH' → agreeOn f H A → ∃ H' : X × ℝ → Y, HomotopyExtension H' f H A rangeH'
\end{minted}
...
The set \lean{rangeH'} has no equivalent in the informal \cref{def:HEP}. It is a set in the codomain type \lean{Y} in which the extendet homotopy \lean{H'} is required to take its values. It forces two additional hypotheses \lean{Set.range f ⊆ rangeH'} and \lean{(smalcyl A).MapsTo H rangeH'}. Without them there could be no such extension, since \lean{H'} extends both \lean{f} and \lean{H}.\\

The gain of carring this set along is that is lets us apply the HEP with a \emph{subset} of a type as codomain without ever forming the corresponding subtype. The application we have in mind is the retaction criterion. There the HEP is applied with \lean{Y := X × ℝ} and \lean{rangeH' := anchor A}, so that the extension it produces is a self-map of \lean{X × ℝ} whose image lies in the anchor. This is precisely what the formalisation of a retraction onto the anchor requires, as discussed later on. \\
In most applications one takes \lean{rangeH'} to be the whole codomain, written  \lean{@ Set.univ Y} in Lean. The two additional hypotheses become statements that \lean{tauto} solves. Since this extra work is negligible, we prefer this definition over the shorter variant discussed next. \\

Because \lean{rangeH'} has no informal counterpart, one could ask whether it changes the notion being defind. To settle this we also formalise the literal reading of \cref{def:HEP}, in which the extension is only required to be a map into \lean{Y}
\begin{minted}{lean4}
...
def HEPY (X : Type u) [TopologicalSpace X] (A : Set X) : Prop :=
  ∀ (Y : Type u) [TopologicalSpace Y], ∀ (f : X → Y), ∀ (H : X × ℝ → Y), Continuous f → ContinuousOn H (smalcyl A) → agreeOn f H A → ∃ (H' : X × ℝ → Y), HomotopyExtensionY H' f H A
\end{minted}
...
In the definition the codomain \lean{Y} is quantified over \lean{Type u}, the same univers as \lean{X}.  This is foreced by the way we use the definition.\\
A quantifier \lean{∀ (Y : Type v)} can only be filled with codomaines lying in exactly that universe. All codomains we ever need, however, are built from the space \lean{X} itself and hence live in its universe. In the retraction criterion, for instance, the codomain is \lean{X × ℝ}, which lies in \lean{Type u}. Quantifying over a lower or higher universe would make the definition inapplicable to such instances. \\
There is a short argument why the definition with this restriction on the universe of \lean{Y} is equivalent to a more general one: the retraction criterion shows that \lean{HEP}, at least in the case that the subspace is closed, is equivalent to the existance of a retraction, and that statement does not mention the codomain \lean{Y} or its universe at all. 
...
\begin{minted}{lean4}
lemma PartialHomeomorph_HEP' {X1 X2 : Set X} (hX : X2 ⊆ X1) {Y1 Y2 : Set Y} (hY : Y2 ⊆ Y1) (hHEP : HEP' X1 X2) (f : PartialHomeomorph X Y) (source : X1 = f.source) (target : Y1 = f.target) (h2 : f '' X2 = Y2) (hY2closed : IsClosed Y2) : HEP' Y1 Y2 := by ...    
\end{minted}
A \lean{PartialHomeomorph X Y} in mathlib is a map between the ambient types \lean{X} and \lean{Y} together with a distinguished source and target on which it is a homeomorphism. The hypotheses \lean{source}, \lean{target} and \lean{h2} identify the two pairs of spaces with these data, and they are stated as equations so that they can be removed by \lean{subst} in the first step of the proof. Afterwards the whole argument is phrased in terms of \lean{f.source}, \lean{f.target} and \lean{f'' X2}, and no transport along the equations is needed anywhere else. Here the ambient formulation of the retraction criterion pays off. We apply \lean{if_HEP_then_retraction'} to \lean{hHEP} and obtain a retraction of \lean{smalcyl f.source} onto \lean{smalanchor f.source X2}. Since this retraction is  a self map of \lean{X × ℝ}, we can conjugate it with with the partial homeomorphism, which is likewise a map between the ambient types. In the first coordinate we precompose with \lean{f.symm} and postcompose with \lean{f}. If only the unprimed criterion were available, the retraction would live on \lean{↑X1 × ℝ}, and we would first have to turn \lean{f} into a homeomorphism \lean{↑f.source ≃ₜ ↑f.target} and work with subtype coercions. \\
Note, that only \lean{Y2} is required to be closed. This is because extracting the retraction from \lean{hHEP} is the forward implication of the retraction criterion, which does not require closedness.
...
Instead of constructiong this partial homeomorphism explicitly, we obtain it from the following mathlib lemma. It states that for a convex bounded set with nonempty interior in a real normed space there is a homeomorphism of the ambient space with itself that carries the interior, the closure and the frontier of the set onto the open unit ball, the closure and the unit sphere respectively. ToDo indirektes Zitat aus Mathlib (auch das folgende lemma) -- Quelle hinzufuegen
\begin{minted}{lean4}
...
def SkeletonProjection (m : ℕ) : ungluedSkel X C m → skeletonLT X m := Sum.elim (fun c ↦ ⟨c, by ... ⟩) (fun ⟨n, i, x⟩ => ⟨map n i x, by ...⟩)
\end{minted}
...
\begin{minted}{lean4}
def sumMap (m : ℕ) :
    (ungluedSkel X C (m + 1)) × unitInterval → skeletonLT X (m+1) × ℝ := fun p ↦ p.1.elim
...
\begin{minted}{lean4}
    Continuous fun (p : (ungluedSkel X C m) × [0,1]) => r (SkeletonProjection X (m + 1) p.1, p.2) 
\end{minted} 
...
\clearpage
\section{Conlusion} \label{section:conclusion}
We tried different ways of formalising the definition of the homotopz extensionproperty and the retraction criterion. The main subject of discussion was, the consideration of how much to work with subtypes. Where it brings advantages and for which applications a phrasing with subtypes is more applicable. \\ 
 
There are the two versions \lean{HEP} and \lean{HEP'}, both of which are used in this formalisation project. \lean{HEP} is applied to a type together with a subset of it, whereas for \lean{HEP'} both spaces are subsets of the same ambient type. We consider the primed version \lean{HEP'} to be more usefull one to work with, optionally with the modification discussed of spelling it out with maps defined on the ambient space. This does not mean that \lean{HEP'} is preferable on counts, since the statements become longer and also the proof of the retraction criterion would have been longer and more tedious than the presented one.\\
For the retraction criterion there are two versions aswell. In \lean{retraction_criterion_closed}, the retraction lives on the product with the bigger space of the HEP, whereas in \lean{retraction_criterion_closed'} the retraction is defined on a product with the ambient type. As discussed in \cref{subsec:lean:retractioncriterion}, both versions are used in this project in a way where they clearly outstand the other version. 

The most obvious point to continue this project would be to generalize the HEP for all relative CW complexes. To formalise the inductive construction of the retraction maps in (ToDo: cref: HEP fuer rel CW), I would recommend working with retractions on the ambient space, which can be composed well together without any coercions. This will give another application of \lean{retraction_criterion_closed'}, where we benefit from the formulation avoiding subtypes. \\
Furthermore one could formalise the retraction criterion without closedness assumption. A consequence of this would be, that we can also drop the assumption in \lean{PartialHomeomorph_HEP'}. 
