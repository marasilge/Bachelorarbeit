...
\setminted{
  ...
  tabsize       = 2,
  fontsize      = \small,
  escapeinside  = «»,   % NICHT || benutzen: Lean schreibt {p | ...}
}
...
This thesis is about the formalisation of the homotopy extension property (HEP) in the formal language of the proof assistant Lean, working towards the result that every relative CW-complex satisfies the HEP. A proof assistant is a software system that checks every single step of a mathematical proof in detail. Lean comes with a mathematical library called mathlib. Both, the language and its library are introduced in \cref{subsec:mathlib}. \\
Relative CW-complexes are already a part of mathlib, the homotopy extension property, however, is not. Mathlib contains the class \lean{HomotopicalAlgebra.CategoryWithCofibrations}, which is a closely related concept, but no formalisation of the HEP that would allow us to work with it explicitly and to prove it for concrete pairs of spaces. This is what is done in this thesis. % Klingt zu sehr so, als wollte ich das zu mathlib machen, aber das ist nicht das Ziel.
...
Formalising a proof has several effects. The first is verification: the proof assistant accepst an argument only once every step has been caried out, which leaves no room for a case that has been overlooked or a hyothesis that is used without being stated. That way results become available to others, since anyone who builds on a formalised statement can rely on its proof being free of errors. What a formal proof cannot guarantee is that the statement says what one intended it to say. We ran into a such point in the closedness assumption of the retraction criterion, which we discuss in \cref{subsec:lean:retractioncriterion}. \\
The second point is a personal one. Formalising these arguments made me understand them far more deeply and precisely than following them in a lecture had. The retraction criterion is my clearest example. Only while formalising it did I notice that the closedness assumption from the given argument is needed in one of the two directions and not in the other. This is something I had not been aware of before, and this is why the corresponding lemma is split in two. \\

\subsection*{Outline}
Throughout this work, unless specified otherwise, every space carries a topology and every map is continuous. \\
\Cref{section:math} gives an overview of the mathematics behind the formalisation. We give the definition of the homotopy extension property in several equivalent phrasings and prove the retraction criterion, which is the tool we use to show that a pair of topological spaces satisfies the property, together with two results on its preservation. Afterwards we tirm to the instance we are interested in, namely that relative CW-complexes satisfy the homotopy extension property. \\
The following chapter \cref{section:Lean} is about Lean and mathlib. Apart from  a short general overview, we highlight three concepts that are relevant for this project, namely the difference between sets and subtypes, structures, and junk values. We briefly introduce mathlib itself and the part of it concerning relative CW-complexes, which our own formalisation builds on. The formalisation is the subject of \cref{section:formalisation} and makes up the main part of this thesis.\\
Based on short excerpts of the Lean code, we explain and assess the decisions made along the way. The formalisation follows the mathematical chapter up to the proof of the homotopy extension property for finite dimensional relative CW complexes. The general case is not covered. \\
...
The formalisation of this project is not based on a single source. The overall structure follows the lecture notes of the course Topology I, taught by Markus Hausmann at the University of Bonn in the winter term 2025/26. These notes are not publicly available. Since the chapter on the homotopy extension property closely follows the (German) lecture notes Algebraische Topologie by F. Waldhausen (Bielefeld) \cite{WAL}, we cite this instead wherever a specific argument is taken from them.
...
A pair of topological spaces $(X,A)$ with $A \subseteq X$ has the \emph{homotopy extension property (HEP)} if for every topological space $Y$, every map $f \colon X \to Y$, and every homotopy $H \colon A \times I \to Y$ satisfying $H(a,0) = f(a)$ for all $a \in A$, there exists a homotopy $H' \colon X \times [0,1] \to Y$ such that
\begin{align*}
    H'(x,0) &= f(x)    && \text{for all } x \in X, \\
    H'(a,t) &= H(a,t)  && \text{for all } (a,t) \in A \times [0,1].
\end{align*}
\end{definition}
The two extreme cases are immediatate, but we record them separately because we will use one of them later as the base case of an induction.
...
\begin{proof} 
    ...
    \] 
    Both maps are continuous: $H$ is the restriction of $g$ to the subspace $A \times I$, and $f$ is the restriction of $g$ to $X \times \{0\}$, composed with the homeomorphism $X \to X \times \{0\}$, $x \mapsto (x,0)$. They are compatible in the sense of \cref{def:HEP}, since $H(a,0) = g(a,0) = f(a)$ for all $a \in A$. Applying the HEP of $(X,A)$ yields a homotopy $H' \colon X \times I \to Y$ with $H'(x,0) = f(x) = g(x,0)$ and $H'|_{A \times I} = H = g|_{A \times I}$. Hence $G \coloneqq H'$ restricts to $g$ on all of $(X \times \{0\}) \cup (A \times I)$. \\
...
\end{proof}
Combining the two lemmas gives the \emph{retraction criterion}, which is the tool we use in several of the following proofs. In the lecture notes, the criterion is stated only for closed subspaces $A \subseteq X$. In \cite{Tammo} if follows the same structure as presented here, assuming closedness for one of the implications. Hatcher \cite[Chapter 0]{HAT01} states the equivalence without this assumption, but for the implication from the existence of a retraction to the HEP he gives a proof under the additional hypothesis that $A$ is closed, referring to the appendix \cite[Appendix]{HAT01} for the general case, where a detailed proof following an argument of \cite{STM} is given. Our formulation follows the same pattern as \cref{lem:HEPextension}. We impose closedness only on the implication that actually needs it. \\
...
\begin{remark}
The assumption that $B$ be closed in $Y$ is not needed for the statement itself. The HEP is preserved under homeomorphisms of pairs without any further hypotheses. It is used only in the proof given above, where closedness is required to apply the given above version of the retraction criterion \cref{prop:retractioncriterion}. This proof is given here, since this is the one formalised.
\end{remark}
...
The following proposition states, that the HEP behaves transitive for two pairs. In the language of cofibrations, this statement asserts that a composite of cofibrations is again a cofibration, and is proofen here: (ToDo: Quelle einfügen) %(ToDo: vermutlich hier: https://hbz-ulb.primo.exlibrisgroup.com/discovery/fulldisplay?docid=cdi_springer_books_10_1007_BFb0083031&context=PC&vid=49HBZ_ULB:DEFAULT&lang=de&search_scope=MyInst_and_CI&adaptor=Primo%20Central&tab=Everything&query=any,contains,tom%20Dieck,%20Algebraic%20Topology)
...
The follwoing definition of relative CW complexes is a modified version of the definition of CW-complexes given in the bachelor thesis of Hannah Scholz \cite{SCH01}. She formalised the historical definition of CW-complexes, presented by Whithead in \cite{WH}, and this formalisation is now available in mathlib as \emph{Classical CW-Complex}. We present her definition because it provides exactly the structures our formalised proofs rely on: explicit characteristic maps together with the weak topology, rather than an inductive description via attaching cells.
...
\[
    ...
    \end{cases}
\] 
    The two cases agree on the sphere $\norm{x} = 1$, where they ask for exactly the compatibility assumption. Both domains are closed in $D_2^m$, so $h$ is continuous by the pasting lemma. Using $h$ we define
    \[
...
Since the definition of CW-complexes in mathlib uses $n$-dimenstional cubes rather than closed balls as the domain of the caracteristic maps, we restate the HEP for cubes relative to their boundary. This result without proof can be found as example (5.1.4) in \cite{Tammo}. There it is claimed for the pair $(I^n, \partial I^n)$. \\
-- ToDo: Question: eigentlich nicht ist der domain nicht der cube, sondern es ist ein partial homeomorphismus mit source cube. Muss man da präzieser sein, oder reicht das, weil es hier nur um die Motivation geht? \\
...
\subsubsection{HEP for consequitive skeletons} \label{subsubsec:HEP_skel}
...
$S \cap C$ is closed by the above. For $n \le m$ we have $S \cap \closedCell{n}{i} = Q_i^n\big((Q_i^n)^{-1}(S)\big) $, which is closed as the image of a closed set of a compact space $D_i^n$ under a continuous map into a Hausdorff space. For $n > m$ we use a variant of the closedness characterisation, which is available in mathlib as \lean{Topology.RelCWComplex.isClosed_of_isClosed_inter_openCell_or_isClosed_inter_closedCell}. By this it suffices to consider the intersection of $S$ with the open cells $\openCell{n}{i}$. Since $S \subseteq X_m$ is disjoint from every open cell of dimension greater than $m$, these intersections are empty and hence closed.
\end{proof}
...
{Z_m \times [0,1]} \arrow[dd, "{P_m \times id_{[0,1]}}"] \arrow[rrr, "f_m"] &  &  & {X_m \times \{0\} \cup X_{m-1} \times [0,1]} \\
                                                                                    &  &  &                                              \\
{X_m \times [0,1]} \arrow[rrruu, "\exists r^{Skel}_m"', dashed]             &  &  &                                             
\end{tikzcd}
\end{center}
...
Now let $(z,t) \in Z_m \times I$ be a point of the second case, so $z \in C$ or $z \in D_i^n$ for some $n < m$. In either case $P_m(z) \in X_{m-1}$, hence $f_m(z,t) \in X_{m-1} \times I$.
    ...
    \emph{(ii).} 
The space $Z_{m} \times I$ is homeomorphic to $(C \times I) \amalg \coprod_{n \le m} \coprod_{i\in I_n}  (D_i^n \times I)$, via the canonical map. Under this homeomorphism, $f_m$ is defined on a coproduct and not a product space anymore. $f_m$ restricts on each summand to a composite of continuous maps. A map out of a coproduct is continuous if and only if all its restrictions to the summands are. Hence $f_m$ is continuous.

    \emph{(iii).}
Let $(z,t)$ and $(z',t') \in Z_m \times I$ with $P_m \times id_I (z,t) = P_m \times id_I (z',t')$. $P_m \times id_{[0,1]}$. Since $P_m \times id_I$ is the indentity on the second coordinate, we directly get $t = t'$ and $P_m(z) = P_m(z')$. We must show, that $f_m (z,t) = f_m (z',t)$. \\
If neither $z$ nor $z'$ lies in a ball of dimension $m$, both are in the second case of the definition and $f_m(z,t) = (P_m \times id_I)(z,t) = (P_m \times id_I)(z',t) = f_m(z',t)$. The same conclusion holds if $z$ or $z'$ lie in $\boundary D_i^m$ for some $i \in I_m$, since $r_i^m$ is a retraction it is the identity on the boundary and $f_m$ again agrees with $P_m \times id_I$. So we may assume that $z \in \interior D_i^m$ for some $i \in I_m$, and it suffices to show $z = z'$. \\
We have $P_m (z) = Q_i^m(z) \in \openCell{m}{i}$. Suppose $z'$ did not lie in $\interior D_i^m$. If $z' \in \interior D_j^m$ for some $j \ne i$, then $P_m(z') \in \openCell{m}{j}$, which is disjoint from $\openCell{m}{i}$ by condition (2) of \cref{def:CWComplex}. In all remaining cases — that is, if $z' \in C$, or $z'$ lies in a ball of dimension less than $m$, or in the boundary of a ball of dimension $m$. Condition (3) gives $P_m(z') \in X_{m-1}$, which is disjoint from $\openCell{m}{i}$ as well. Both contradict $P_m(z) = P_m(z')$, so $z' \in \interior D_i^m$. Since $\restrict{Q_i^m}{\interior D_i^m}$ is a homeomorphism onto $\openCell{m}{i}$ and hence is injective, $P_m(z) = P_m(z')$ forces $z = z'$.
\end{proof}

The idea of the following proof is from {\cite[page 40]{WAL}}.
...
ToDo: in der Quelle ist in diesem Beweis ein Fehler... nach anderer Quelle suchen. Question: Soll ich da noch drauf aufmerksam machen?
\begin{lemma}  [cf.\ {\cite[page 40 f.]{WAL}}] \label{HEP_CW}
    Let $(X,C)$ be a relative CW complex. Then $(X,C)$ has the HEP.
\end{lemma}

\begin{proof}[Sketch]
    We use the retraction criterion, which applies since $C$ is closed in $X$. \\
    In a first step the maps $r_m$ are constructed inductively for all $m \in \mathbb{N} \cup \{-1\}$. 
    \[
...
\subsubsection{Sets and Subtypes} \label{subsubsec:sets}
...
Closely related is the type \lean{Set α} of subsets of \lean{α}, which is defined as \lean{α → Prop}. These two notions are connected by a coercion. Every \lean{S : Set α} can be read as a type \lean{↑S}, namely the subtype \lean{{x : α // x ∈ S}}, and conversely every term of \lean{↑S} has an underlying term of \lean{α}, obtained by \lean{Subtype.val}. Both directions are inserted automatically by Lean where needed. \\
...
\subsubsection{Structures} \label{subsubsec:structures}
...
\subsubsection{Junk Values} \label{subsubsec:junkvalues}
...
We introduce those aspects of mathlib's formalisation of CW-complexes that are relevant for us. The reference for this section is the part \lean{Mathlib.Topology.CWComplex.Classical} of mathlib \cite{mathlib}. All notions discussed below live in the namespace \lean{Topology.RelCWComplex}, which we open in our files, so that we write \lean{cell}, \lean{map} or \lean{skeletonLT} without the prefix.\\ 

First of all, a relative CW-complex is not defined as a pushout, but instead lives in an ambient type. That is what motivated the definition \lean{HEP'}. A relative CW-complex $(X,C)$ is declared in the following way. 
\begin{minted}{lean4}
variable {Y : Type u} [TopologicalSpace Y] (X : Set Y) {C : Set Y} [RelCWComplex X C]
\end{minted}
Here the complex \lean{X} and its base space \lean{C} are both sets of the ambient type \lean{Y}. This is the third of the variable blocks fixed in \cref{subsec:lean:notation}. Mathlib's definition does not assume the ambient space to be Hausdorff. Nevertheless, for many statements about complexes we have to add the instance \lean{[T2Space Y]}. \\
...
These characteristic maps are not maps from the closed ball to the closed cell, but maps between the ambient types. They are partial equivalences with source the open ball and target the open cell, since these are the sets between which we have a bijection, and they are continuous on the closed ball, since that is where their definition has a mathematical meaning.\\
When saying that the source of the partial equivalence is an open ball, one has to be careful. In the definition this ball lies in the type \lean{Fin m → ℝ}, which is a formalisation of the $m-$dimensional real vector space carrying the maximum norm. Hence the ball is something we would usually rather call a cube. That is why it does not suffice to prove the HEP for a ball in an $m-$dimensional Euclidean space \lean{EuclideanSpace ℝ (Fin m)}, but the result has to be transferred to cubes, which we do in \cref{subsec:lean:ballcube}.\\
The topology of a complex is described through its closed sets. A subset of \lean{X} is closed if and only if its intersection with the base space and with every closed cell is closed there. Note that with this definition the description of the complex as a quotient of the disjoint union of the base space with the closed balls, which for a definition by a pushout would be the definition itself, becomes a statement that has to be proved. We do exactly that in \cref{subsubsec:lean:skel:quotient}. \\
...
Throughout the formalisation project we use the follwoing abbreviations.
...
These four abbreviations name the subsets that occur in the definition of the HEP and in the retraction criterion. The set \lean{cyl X} is the cylinder of hight one over the type \lean{X}, whereas \lean{smalcyl A} is the cylinder only over a set \lean{A} of type \lean{Set X}. The set \lean{anchor A} is the union of the cylinder over \lean{(A : Set X)} together with the bottom of the cylinder \lean{cyl X}. Finally, \lean{smalanchor X A} is the corresponding set for the case that \lean{(X A : Set Y)} are subsets of an ambient type \lean{Y}. It  is the union of the cylinder over \lean{A} with the bottom of \lean{smalcyl X}. All four are declared as \lean{abbrev} rather than \lean{def} and are therefore reducible, so that elaboration and \lean{simp} see through them.  \\

\subsection{HEP Definition} \label{subsec:lean:definition}
Already in the mathematical part, the definition of the homotopy extension propert was long and technical and its formalisation is no shorter. We can now turn to it.

\begin{minted}{lean4}
def agreeOn (f : X → Z) (H : X × ℝ → Z) (A : Set X) : Prop := ∀ (a : X), a ∈ A → f a = H (a,0)

structure HomotopyExtension (H' : X × ℝ → Z) (f : X → Z) (H : X × ℝ → Z) (A : Set X) (rangeH' : Set Z) : Prop where
...
def HEP (X : Type u) [TopologicalSpace X] (A : Set X) : Prop :=
  ∀ (Z : Type u) [TopologicalSpace Z] (rangeH' : Set Z), ∀ (f : X → Z), ∀ (H : X × ℝ → Z), Continuous f → Set.range f ⊆ rangeH' → ContinuousOn H (smalcyl A) → (smalcyl A).MapsTo H rangeH' → agreeOn f H A → ∃ H' : X × ℝ → Z, HomotopyExtension H' f H A rangeH'
\end{minted}
...
Secondly, the structure \lean{HomotopyExtension} bundles the properties the desired homotopy should have: continuity on the cylinder, that its values lie in the prescribed set, and that it agrees with the two given maps. Using a structure rather than a conjunction has the advantage that it comes with projections for the individual fields, which is what makes the applications readable. \\

This definition of \lean{HEP} is close to the informal \cref{def:HEP}, apart from the three aspects elaborated below. Before tuning to that and as a first sanity check that the defintion is usable at all, we prove that every space has the HEP relative to itself and relative to the empty set. The lemma \lean{HEP_self} will serve as the base case of the induction in \cref{subsec:lean:relCW}.
\begin{minted}{lean4}
...
This does not change the object being defined. The field \lean{ContinuousOn H' (cyl X)} constrains \lean{H'} only onto the cylinder. So what \lean{H'} does outside the unit interval is irrelevant. This field shifts the focus onto exactly what the definition needs. The choise not to work with subtypes is not free of cost. For example in the backward direction of \lean{lemma retraction_criterion_closed'} we will have to precompose with a projection onto $[0,1]$ in order to make the \lean{MapsTo} field applicable, as discribed in \cref{subsec:lean:retractioncriterion}.\\

\subsubsection*{The set \lean{rangeH'} and the variant \lean{HEPY}}

The set \lean{rangeH'} has no equivalent in the informal \cref{def:HEP}. It is a set in the target type \lean{Z} in which the extendet homotopy \lean{H'} is required to take its values. It forces two additional hypotheses \lean{Set.range f ⊆ rangeH'} and \lean{(smalcyl A).MapsTo H rangeH'}. Without them there could be no such extension, since \lean{H'} extends both \lean{f} and \lean{H}.\\

The gain of carring this set along is that is lets us apply the HEP with a \emph{subset} of a type as codomain without ever forming the corresponding subtype. The application we have in mind is the retaction criterion. There the HEP is applied with \lean{Z := X × ℝ} and \lean{rangeH' := anchor A}, so that the extension it produces is a self-map of \lean{X × ℝ} whose image lies in the anchor. This is precisely what the formalisation of a retraction onto the anchor requires, as discussed later on. \\
In most applications one takes \lean{rangeH'} to be the whole target type, written  \lean{@ Set.univ Z} in Lean. The two additional hypotheses become statements that \lean{tauto} solves. Since this extra work is negligible, we prefer this definition over the shorter variant discussed next. \\

Because \lean{rangeH'} has no informal counterpart, one could ask whether it changes the notion being defind. To settle this we also formalise the literal reading of \cref{def:HEP}, in which the extension is only required to be a map into \lean{Z}
\begin{minted}{lean4}
...
def HEPY (X : Type u) [TopologicalSpace X] (A : Set X) : Prop :=
  ∀ (Z : Type u) [TopologicalSpace Z], ∀ (f : X → Z), ∀ (H : X × ℝ → Z), Continuous f → ContinuousOn H (smalcyl A) → agreeOn f H A → ∃ (H' : X × ℝ → Z), HomotopyExtensionY H' f H A
\end{minted}

The lemma \lean{HEP_iff_HEPY} shows that the two definitions are equivalent, so that \lean{rangeH'} is a matter of convenience and not of content. We will not use \lean{HEPY} again in this project. It's only reason to exist is this equivalence. 

\subsubsection*{Universes}

In the definition the target type \lean{Z} is quantified over \lean{Type u}, the same univers as \lean{X}.  This is foreced by the way we use the definition.\\
A quantifier \lean{∀ (Z : Type v)} can only be filled with codomaines lying in exactly that universe. All codomains we ever need, however, are built from the space \lean{X} itself and hence live in its universe. In the retraction criterion, for instance, the codomain is \lean{X × ℝ}, which lies in \lean{Type u}. Quantifying over a lower or higher universe would make the definition inapplicable to such instances. \\
There is a short argument why the definition with this restriction on the universe of \lean{Z} is equivalent to a more general one: the retraction criterion shows that \lean{HEP}, at least in the case that the subspace is closed, is equivalent to the existance of a retraction, and that statement does not mention the target type \lean{Z} or its universe at all. 

\subsubsection*{The variant \lean{HEP'}}
Let us now turn to the variant \lean{HEP'}, which changes the shape of the pair of spaces.
...
The notation \lean{(X ↓∩ A)} means, that we regard \lean{(X ∩ A)} as a set of type \lean{Set ↑X}. Te difference is that in \lean{HEP} the set \lean{A} is a subset of the type \lean{X}, whereas in \lean{HEP'} both \lean{X} and \lean{A} are subsets of one ambient type \lean{Y}.\\
Two consequences of defining \lean{HEP'} through \lean{HEP} are worth noting. First, \lean{HEP'} does not require $A$ to be a subset of $X$. To be precise, we do not check, that the pair $(X,A)$ has tht HEP, but instead the pair $(X, X \cap A)$ does, and this is the intended statment only if $A \subset X$. In some proofs, such as \lean{HEP_trans}, the inclusion therefore appears as an explicit hypothesis.\\
Secondly, \lean{HEP} and \lean{HEP'} are definitionally equal, so results transfer between them for free, als we will see for example when \lean{HEP_disc_boundary'} is proved by \lean{HEP_disc_boundary} alone. \\
...
Note, that in this definition the maps \lean{f} and \lean{H} to be extended are defined on the subtype \lean{↑X}, respectively \lean{↑X × ℝ}. Working with these subtypes caused some extra work in the proof of \lean{retraction_criterion_closed'}. One could avoid it by not defining \lean{HEP'} in terms of \lean{HEP} but writing it out with maps defined on \lean{Y} and \lean{Y × ℝ}. The definition would be longer, since for instance continuity of \lean{f} would become to a \lean{ContinuousOn} statement. Also, the equivalence with \lean{HEP} would have to be proven rather than holding by definition. Nevertheless, it is an interesting modification that should be considered looking for the best formalisation, even tough we do not pursue it further in this work. \\
...
type \lean{X} that is not a retraction on all of its domain. It only "retracts" a subset \lean{B} onto a subset \lean{A}. The four fields translate the classical conditions as as follows. The first property \lean{subset} states that \lean{A} is contained in \lean{B}. Note, that omitting this property would define a more general class of self-maps that need not staisfy $r(B) = A$. The field \lean{continuousOn} requires continuity on the relevant subset $B$ only. Since the codomain of a retraction is supposed to be \lean{A}, the field \lean{mapsTo} requires that \lean{r} sends every element of \lean{B} to an element of \lean{A}. Finally, the classical definition demands that precomposition with the inclusion $A \hookrightarrow B$ gives the identity $id_A$. In our setting there is no inclusion map, since domain and codomain of \lean{r} are the same type \lean{X}, so the condition therefore becomes the requirement that ever element of \lean{A} is fixed by \lean{r}. This is the field \lean{fixesOn}. \\

Recall that the retraction criterion was obtained from two lemmas, and the formalisation follows the same structure. Since \cref{lem:HEPextension} consists of two implications, only one of which requires closedness, we split it into two separate lemmas. The forward implication is called \lean{if_HEP_then_extension}, and the equivalence under the closedness assumption is called \lean{retraction_criterion_closed}. Their proofs are unspectacular and follow the informal argument of \cref{lem:HEPextension} closely, as do the formalisations of \cref{lem:retract} and of the retraction criterion \cref{prop:retractioncriterion} itself. 
...
«\leanout{ if\_HEP\_then\_retraction hHEP' : ∃ (r : ↑X × ℝ → ↑X × ℝ), RetractionOn r (cyl ↑X) (anchor (X ↓∩ A))}»
\end{minted}
Question: I modified the output of the check by adding the type of r by hand. Should I mark that?
...
lemma retraction_criterion_closed' (X A : Set Y) (hAX : A ⊆ X)
    ...
    := by ...
\end{minted}
At this point the ambient-space formulation introduces a discrepancy with the informal statement, which went unnoticed during the formalisation. In \cref{lem:HEPextension} and \cref{prop:retractioncriterion} the hypothesis is that $A$ is closed in $X$, that is, in the subspace topology of $X$. In \lean{retraction_criterion_closed'}, however, both $A$ and $X$ are subsets of the ambient type \lean{Y}, and \lean{hA1 : IsClosed A} asserts that $A$ is closed in
\lean{Y}. This is strictly stronger. \\
...
The two versions of the criterion differ in two independet aspects, which are worth keeping apart. The first is which definition of HEP appears in the statement. As \lean{#check} above shows, this is a matter of style. \lean{HEP} and \lean{HEP'} are definitionally equal, so either criterion could have been stated in terms of either definition. The second aspect is where the retraction lives, on \lean{↑X × ℝ} for a type \lean{X} or on \lean{Y × ℝ} for an ambient type \lean{Y}, and here the two forms are not interchangeable. While we prefered working with tyes instead of subtypes for the definition of the homotopy extension property, here we experienced both versions valuable in practice. I will give an example for each version: \\
...
The primed version on the other hand is what \lean{PartialHomeomorph_HEP'} needs, as we discuss in \cref{subsec:lean:preservation}. A partial homeomorphism is a map between two ambient types, so a retraction defined on the type rather than on a subtype can be used more directly.  


\subsection{Preservation of the HEP}  \label{subsec:lean:preservation}
Besides the retraction criterion we formalised two more results that are useful for proving that a pair of topological spaces has the HEP. The first is that the HEP is preserved under partial homeomorphisms.  \\

\begin{minted}{lean4}
lemma PartialHomeomorph_HEP' {Y' : Type u} [TopologicalSpace Y'] {X A : Set Y} (hA : A ⊆ X) {X' A' : Set Y'} (hA' : A' ⊆ X') (hHEP : HEP' X A) (f : PartialHomeomorph Y Y') (source : X = f.source) (target : X' = f.target) (h2 : f '' A = A') (hA'closed : IsClosed A') : HEP' X' A' := by ...
\end{minted}
The two pairs of this lemma are \lean{(X,A)} in the ambient type \lean{Y} and \lean{(X',A')} in a second ambient type \lean{Y'}. A \lean{PartialHomeomorph Y Y'} in mathlib is a map between the ambient types \lean{Y} and \lean{Y'} together with a distinguished source and target on which it is a homeomorphism. The hypotheses \lean{source}, \lean{target} and \lean{h2} identify the two pairs of spaces with these data, and they are stated as equations so that they can be removed by \lean{subst} in the first step of the proof. Afterwards the whole argument is phrased in terms of \lean{f.source}, \lean{f.target} and \lean{f '' A}, and no transport along the equations is needed anywhere else. Here the ambient formulation of the retraction criterion pays off. We apply \lean{if_HEP_then_retraction'} to \lean{hHEP} and obtain a retraction of \lean{smalcyl f.source} onto \lean{smalanchor f.source A}. Since this retraction is  a self map of \lean{Y × ℝ}, we can conjugate it with with the partial homeomorphism, which is likewise a map between the ambient types. In the first coordinate we precompose with \lean{f.symm} and postcompose with \lean{f}. If only the unprimed criterion were available, the retraction would live on \lean{↑X × ℝ}, and we would first have to turn \lean{f} into a homeomorphism \lean{↑f.source ≃ₜ ↑f.target} and work with subtype coercions. \\
Note, that only \lean{A'} is required to be closed. This is because extracting the retraction from \lean{hHEP} is the forward implication of the retraction criterion, which does not require closedness.

The second result is a transitivity property: if the pairs $(A_1,A_2)$ and $(A_2,A_3)$ both satisfy the HEP, then so does the pair $(A_1,A_3)$.
\begin{minted}{lean4}
lemma HEP_trans (A1 A2 A3 : Set X) (h21_sub : A2 ⊆ A1) (h12_hep : HEP' A1 A2) (h32_sub : A3 ⊆ A2) (h23_hep : HEP' A2 A3) : HEP' A1 A3 := by ...
\end{minted}

For the formalisation of this proof we had a choice between two approaches. Instead of applying the retraction criterion to all three \lean{HEP} statements and constructing the desired retraction from the two given ones, we proved the lemma directly from the definition. This turned out to be the shorter way, and a further advantage of the choice is that no closedness assumptions are needed, whereas applying the retraction criterion would have required them. \\

In retrospect, \lean{PartialHomeomorph_HEP'} could be proven in the same manner, by checking the definition and construction the extenden homotopy explicitly rather than going through the retraction criterion. In that case the closedness assumption on \lean{A'} could be dropped as well. With the current definition of \lean{HEP'}, however, in which the maps we wish to extend live on subtypes, such a proof would be less pleasant to carry out.
...
We start with the balls. As in the proof of \cref{thm:HEPball}, we first construct the auxiliary functionion \lean{aux_fun}, which combines the information of the given maps \lean{f} and \lean{H}. This function is then used to define the desired homotopy \lean{H'} explicitly, with which we can check the definition of \lean{HEP}. Following the notation of \cref{subsec:lean:notation}, the target type is \lean{Z} and \lean{rangeH' ⊆ Z} bounds the range of the maps; here \lean{Z} has to lie in \lean{Type 0}, since the pair of this section does. The maps \lean{f} and \lean{H} are declared as variables as well, because every lemma of the file speaks about them.
...
The \lean{Metric.sphere} in Lean has type \lean{Set α}, where \lean{α} is the type of the center, in our case the zero of an $m-$dimenstional Euclidian space. By the way \lean{HEP} is defined, the sphere in \lean{HEP_disc_boundary} is a subset of the closed ball. so we have to add the \lean{by simp} proof in order to construct the element of the subtype. For \lean{HEP'}, on the other hand, both spaces live in the same ambient space. No proof has to be supplied, and  Lean interprets the zero as the origin of $\mathbb{R}^m$. Here we see an advantage of the \lean{HEP'} definition in action, since we can reduce the use of subtypes. \\
In \cref{subsubsec:mathlibCW} we saw that the preimage of a closed cell in a CW-complex is a cube not a ball. To prove the \lean{HEP} for these cubes, we want to use the fact that an $m-$dimensional ball is homeomorphic to a cube of the same dimension. \\
Instead of constructiong this partial homeomorphism explicitly, we obtain it from the following mathlib lemma which can be found at \lean{Mathlib.Analysis.Convex.GaugeRescale} in \cite{mathlib}. It states that for a convex bounded set with nonempty interior in a real normed space there is a homeomorphism of the ambient space with itself that carries the interior, the closure and the frontier of the set onto the open unit ball, the closure and the unit sphere respectively. 
\begin{minted}{lean4}
...
The composition of these two homeomorphisms is in particular a partial homeomorphism from \lean{EuclideanSpace ℝ (Fin m)} to \lean{Fin m → ℝ} with source the closed ball and target the closed cube. Applying the lemma \lean{PartialHomeomorph_HEP'} now proves 
\begin{minted}{lean4}
...
\subsection{HEP for consecutive skeletons}  \label{subsec:lean:skel}
...
Now we continue with the second step in the proof of the HEP for relative CW-complexes, namely that every pair of two consecutive skeletons satisfies the HEP. The formalisation of this result builds on the following mathlib lemma, which takes the place of the theorem  stating that the product of a quotient map with the identity on a locally compact space, is again a quotient map (\cite{thm:quotientproduct}). The lemma if from \lean{Mathlib.Topology.CompactOpen} in \cite{mathlib}.
...
The theorem tells us that a map \lean{g} defined on the product of a quotient space with a locally compact space is continuous, provided that its composition with the quotient map in the first factor is. We want this \lean{g} to be the map that yields the HEP for two consecutive seketons. Mathematically it is the retraction $X_m \times [0,1]$ onto $X_m \times \{0\} \cup X_{m-1} \times [0,1]$. In the formalisation it takes the shape \lean{skeletonLT X (m+1) × unitInterval → skeletonLT X (m+1) × ℝ}, which will be discussed later on. For now, let us take a closer look at which functions we need to define and which results we have to proof in order to get there. \\
...
First of all we need a quotient map onto the skeleton \lean{skeletonLT X m} called \lean{f} in the theorem above. For this we use the fact that we can "unglue" the skeleton into the base space and the disjoint union of closed balls, or cubes to be precise, in all dimensions less than $m$. The identity map on the base space and the caracteristic maps on the closed balls assemble into the map \lean{SkeletonProjection}.

\begin{minted}{lean4}
abbrev ungluedSkel (m : ℕ) :=
  C ⊕ (Σ (n : Fin m) (_ : cell X n), (closedBall (0 : Fin n → ℝ) 1))

def SkeletonProjection (m : ℕ) : ungluedSkel X m → skeletonLT X m := Sum.elim (fun c ↦ ⟨c, by ... ⟩) (fun ⟨n, i, x⟩ => ⟨map n i x, by ...⟩)
\end{minted}

To prove that this indeed defines a quotient map, we have to check that it is surjective and continuous and that it induces the topology on the skeleton. The skeleton is a subcomplex of the relative CW-complex, and its topology is described in terms of the closed sets: A subset of the subcomplex is closed precisely when its intersection with the base space is closed there and its intersection with each closed cell is closed in that cell. \\
\\ 
The surjectivity proof is straightforward. A point of the skeleton lies either in the base space or in an open cell, and in the two cases it has a preimage in \lean{C} respectively in the closed ball belonging to that cell. Continuity is not much work either, since \lean{SkeletonProjection} is assembled from continuous functions on a disjoint union. Proving that the topology on the skeleton is the induced one required more effort. \\
...
Now we can apply the lemma \lean{Topology.RelCWComplex.isClosed_of_isClosed_inter_openCell_or_isClosed_inter_closedCell}, which states that \lean{S'} is closed in the relative CW-complex if its intersection with the base space is closed and if for every cell either \lean{S' ∩ openCell n j} or \lean{S' ∩ closedCell n j} is closed. We prove the case of the base space in \lean{hClosedBase} and the case of cells of dimension less than $m$ in \lean{hClosedLT}. For higher dimensions the intersection of \lean{S'} with the open cell is empty and hence closed. \\

\subsubsection{Construction of the retraction} \label{subsubsec:lean:skel:constructionretraction}
The next preparation is the construction of the map \lean{r}, which is to be the retraction for two consecutive skeletons. As in the mathematical proof, we do not define the retraction map \lean{r} on \lean{skeletonLT X (m + 1) × [0,1]} directly. Instead we say what it should do on the unglued skeleton, where the base space and the individual cubes are still separate, and then push the resulting map down along the quotient map \lean{SkeletonProjection}. We build this up from the bottom: first the map on all single cubes, calles \lean{cubesretract}, then the map on the product of the whole unglued skeleton and the unit interval, which is called \lean{sumMap} and was called $f$ in the mathematical section, and finally \lean{r} itself.\\

On a cube of top dimension the retraction we need is already available. It is the map \lean{r_cube} that the retraction criterion provides from \lean{HEP_cube_boundary}, the HEP of a cube relative to its boundary proved in \cref{subsec:lean:ballcube}. On cubes of lower dimension nothing should happen. The map \lean{cubesretract} combines the two cases. Apart from the case distinction it only coerces the second coordinate from the unit interval to \lean{ℝ}.
\begin{minted}{lean4}
...
For \lean{cubesretract} we proved lemmas about its continuity, about where it is fixed, and about the set in which its image lies. The last one comes in a version for the lower dimensions and one for the top dimension, matching the two branches of the definition.
...
lemma cubesretract_fixedOn {m : ℕ} {n : Fin (m + 1)} (x : (closedBall (0 : Fin n → ℝ) 1) × unitInterval) :
    ...
    (n = m → x ∈ {z | z.2 = 0 ∨ z.1 ∈ sphere ⟨0, by simp⟩ 1 } → cubesretract n x = (x.1, x.2.1)) := by …
...
lemma cubesretract_mapsto_top {m : ℕ} (n : Fin (m + 1)) (i : cell X n) (p : (closedBall 0 1) × ↑unitInterval) (hn : n = m) :
    (cubesretract n p).2 = 0 ∨ (map n i) (cubesretract n p).1 ∈ skeletonLT X m ∧ (cubesretract n p).2 ∈ unitInterval := by …
\end{minted}

Next we assemble these maps into a map on the product of the whole unglued skeleton times the unit interval.
\begin{minted}{lean4}
def sumMap (m : ℕ) :
    (ungluedSkel X (m + 1)) × unitInterval → skeletonLT X (m+1) × ℝ := fun p ↦ p.1.elim
      (fun c => (⟨c.val, memBase_memSkeletonLT X c.prop (m + 1)⟩, p.2))
      (fun ⟨n, i, x⟩ => (⟨map n i (cubesretract n (x,p.2)).1, cubesretract_mapsto_weak n i (x,p.2)⟩, (cubesretract n (x,p.2)).2))
\end{minted}

The map \lean{sumMap} is defined using \lean{Sum.elim}, which performs a case analysis on a sum type and applies the corresponding function depending on which constructor is present (c.f. \lean{Init.Data.Sum.Basic} \cite{mathlib}), that is, on wether the first coordinate of the point \lean{p} lies in the base space \lean{C} or in a closed ball. In the first case we want to do nothing except apply \lean{SkeletonProjection} to that coordinate, which on the base space is just the identity. We send the point \lean{(c : C)} to itself, but regarded as an element of the skeleton. Since the proof that a point of the base space lies in any given skeleton is needed repeatedly, we made it a lemma of its own, called \lean{memBase_memSkeletonLT}.\\ 
In the second case, where the first coordinate of \lean{p} comes from a closed ball, we first apply \lean{cubesretract} to the point and then postcompose with what \lean{SkeletonProjection × id} would do to a such point. Since the first coordinate, still lies in a closed ball after \lean{cubesretract} has been applied, \lean{SkeletonProjection} amounts to applying the charakteristic map of that closed ball. We have to supply a poof, that the image of this characteristic map really lies in the skeleton.This proof is again a lemma of its own, called \lean{cubesretract_mapsto_weak}, which is derived from the two \lean{mapsto} statements above. \\

It remains to push \lean{sumMap} down from the unglued skeleton to the skeleton itself, which is what \lean{Function.extend} does.

\begin{minted}{lean4}
def r (m : ℕ) : skeletonLT X (m + 1) × unitInterval → skeletonLT X (m + 1) × ℝ :=
  Function.extend (fun p ↦ ((SkeletonProjection X (m + 1) p.1), p.2)) (sumMap X m) (Prod.map id Subtype.val)
\end{minted}

\lean{Function.extend} works in such a way that whenever a point \lean{x ∈ skeletonLT X (m + 1)} has a preimage \lean{p ∈ ungluedSkel × [0,1]} under the map \lean{(fun p ↦ ((SkeletonProjection X (m + 1) p.1), p.2))}, then \lean{r x = sumMap X m}, and otherwise \lean{r x = (Prod.map id Subtype.val) x}. This is not captured in the defintion, but we know that \lean{Skeletonprojection × id} is surjective, so the second case of \lean{Function.extend} could be filled with any junk function, since it will never be applied. One can therefore read the definition of \lean{r} simply as: take any preimage under \lean{SkeletonProjection} and apply \lean{sumMap} to it. That this is well defined is the content of the lemma \lean{factors} in the next section. \\
...
Taking a closer look at the definition of \lean{sumMap}, one sees that the case where \lean{p} comes from the base space or from a closed ball of dimension less than \lean{m}, it does the same as \lean{SkeletonProjection × id}. At least if one ignores the subtype issues. It would have been inconvenient to write this application into the definition of \lean{sumMap}, so instead we proved the two lemmas that state exactly this equality.
...
lemma sumMap_eq_SkelProj_sphereLT {m : ℕ}
    ...
    sumMap m (Sum.inr ⟨n, ⟨i, x'⟩⟩, t) = Prod.map (SkeletonProjection X (m + 1)) Subtype.val (Sum.inr ⟨n, ⟨i, x'⟩⟩, t) := by …
\end{minted}

\subsubsection{Continuity of the retraction} \label{subsubsec:lean:skel:continuityretraction}
Now that all relevant maps are defined, we can turn to the proof that \lean{r} is continuous. As announced, the continuity comes from the theorem \lean{Topology.IsQuotientMap.continuous_lift_prod_left}. Let us see what is left to prove: 
...
\begin{minted}{lean4}
    Continuous fun (p : (ungluedSkel X m) × [0,1]) => r (SkeletonProjection X (m + 1) p.1, p.2) 
\end{minted} 
Showing this comes down to two major steps. The fist is that \lean{sumMap} factors through \lean{(fun p ↦ ((SkeletonProjection X (m + 1) p.1), p.2))}, from which it follows easily that the continuity goal is equivalent to the continuity of \lean{sumMap} is continuous. That continuity is then the second step.\\
We start with 
\begin{minted}{lean4}
lemma factors (m : ℕ) : (sumMap m).FactorsThrough (fun p ↦ ((SkeletonProjection X (m + 1) p.1), p.2)) := by 
    ...
    ...
\end{minted}
Here \lean{x} and \lean{y} are two points of \lean{ungluedSkel × [0,1]} whose second coordinates agree and which have the same image under \lean{SkeletonProjection}. We have to show that \lean{sumMap X m x = sumMap X m y}. In \lean{sumMap_eq_SkelProj_C}, and \lean{sumMap_eq_SkelProj_sphereLT} we already have useful tools for this this goal. One part that required some work is the following: 
\begin{minted}{lean4}
lemma SkeletonProj_inj_top {m : ℕ} {n : Fin (m + 1)} (hnm : ↑n = m) (i : cell X n)
    ...
    y = Sum.inr ⟨n, i, x'⟩ := by ...
\end{minted}
The lemma \lean{SkeletonProj_inj_top} says that if \lean{x} lies in a specific open ball of top dimension and \lean{y} is any other point of \lean{ungledSkel} with the same image under \lean{SkeletonProjection}, then \lean{y} is that very point \lean{x}. \\
The proof of \lean{factors} is long because it splits into many case distinctions, begining with whether \lean{x} lies in the base space or in a closed ball, and considering in each case the same two options for \lean{y} separately. In the case where \lean{x} lies in a closed ball we distinguish further whether it lies in the top-dimesnioal open cell or not. Most of these cases are quite short in themselves. \\
In the mathematical argument it was enough to destinguish whether a point lies in the top-dimensional cell or not. So one may ask why the formalisation splots the situation up so much further. The reason is that we cannot simply say that a point lies in the base space. A point of \lean{ungluedSkel} is given by a constructor, either as \lean{Sum.inl c} for some \lean{c : C} or as as \lean{Sum.inr ⟨n, i, x⟩}, so the case analysis over the construtors has to be made anyway, and it is then easier to treat the resulting combinations seperately. % ToDo / Question : Ist das verstaendlich?  \\
For the continuity of \lean{SumMap} we precompose the map with the homeomorphism 
\begin{minted}{lean4}
def homeoProd (m : ℕ) :
...
In this section we conclude the proof of the HEP for consecutive skeletons. For the proof via the retraction criterion we have to make a small adjustment to the map \lean{r}. As mentioned above the desired retraction sould have \lean{(skeletonLT X (m + 1)) × ℝ} as its domain, whereas \lean{r} is only defined on the unit interval in the second component. The easiest way to extend \lean{r} turned out to be precomposing the second coordinate with the map \lean{ProjIcc}, which was defined in the first file \lean{HEP_definition.lean}. The final definition therefore looks as follows.
...
It only remains to show that \lean{retr_skeleton} is continuous, maps into the right set and is fixed where required. The continuity follows directly from all the work done before and the claims about \lean{mapsTo} and \lean{fixedOn} are first proved for \lean{sumMap} and then transferred to \lean{retr_skel}. 
%(ToDo: man könnte hier noch auf subst eingehen, weil das ein game changer war.)
Altogether this results in the following lemma. 
...
In this last part of the formalisation we arrive at the result that finite dimensional relative CW complexes satisfy the HEP. As the mathematical chapter already showed, the finite dimensional case follows from what we have proven so far by a straightforward induction, whereas the general case requires considerably more work. Due to the limited time for this thesis I had to stop at this point. \\
We first run the induction over the skeleta, and then show that a finite dimensional complex is one of its own skeleta. The induction looks in Lean as follows.
...
The base case is the lemma \lean{HEP_self}, which we proved at the very begining, just after defining the HEP. Getting there takes the rewrite that the base space is the same as the $(-1)-$skeleton. For the induction step we combine the induction hypothesis with the HEP for two consecutive skeletons via the trasitivity lemma \lean{HEP_trans}. \\
The corresponding statement for \lean{skeleton} then follows from the one for \lean{skeletonLT}, since \lean{skeleton X m} in \lean{skeletonLT X (m+1).}\\
\begin{minted}{lean4}
...
It remains to show that for a finite dimensional complex there is an $m \in \mathbb{N} \cup \{-1\}$ witht \lean{X = skeletonLT X m}. This is the content of the following lemma.
\begin{minted}{lean4}
...
Putting the two steps together, the final proof is short.
\begin{minted}{lean4}
...
This completes the three steps announced at the beginning: the \emph{HEP} for a ball and a cube relative to their boundary, for two consecutive skeleta, and finally for a finite dimensional relative CW complex relative to its base space.

\clearpage
\section{Methodes and Usage of AI} \label{section:methodes}
...
All submitted Lean code is written by myself, except a short definition together with a related lemma in the file \lean{HEP_skeleton.lean}. These approximatly twenty lines marked accordingly in the formalisation. This lemma was a tool that could have been a part of mathlib. It is not specific to the constructions of this project.\\
The rest of the formalisation is my own work, but it would in some parts look different without the use of AI. I frequently asked Claude when a proof got stuck or an error message was difficult to interpret. One example is the tactic \lean{subst}, which simplified the proof of \lean{SkeletonProjectionIsQuotientMap} a lot and later allowed me to shorten several earlier drafts by a noticeable number of lines. Another is the observation that consecutive \lean{exact} statements can be combined into a single \lean{exacts [...]}. \\
For larger design questions I found AI less helpful. The pushout construction for the retraction of consecutive skelta used in the lecture notes, for instance, did not match the formalisation of CW-complexes in mathlib that I wanted to work with. I asked Claude for help at this point. Instead of working within the subset setting of the formalised CW-complexes, which is the solution presented earlier, it suggested a longer and more technical way that did not seem realisable to me. The biweekly meetings with Floris van Doorn were the place where such decisions were actually resolved.\\
I wrote the text of this thesis in full myself in a first step. In a second pass I went through it paragraph by paragraph and improved the language, along with some inaccuracies, with the help of Claude. Every reformulation was reviewed before I adopted it, and I discarded those that shifted the meaning of what I had written. AI was also used to set up the configuration for typesetting Lean code in this \LaTeX{} document.
...
One result deserves to be mentioned separately. The lemma \lean{SkeletonProjectionIsQuotientMap} states that every skeleton of a relative CW-complex is a quotient of the disjoint union of its base space with the closed cubes of the cells it contains. Mathlib defines relative CW-complexes as a set in an ambient type rather than as a pushout. This result can extend the available definition by a new perspective, which might be usefull to proof more results about CW-complexes in the future.\\ %ToDenken: was genau koennte das sein?

In this project there are the two definitionally equal versions of the homotopy extension property, \lean{HEP} and \lean{HEP'}, both of which are used in this formalisation project. \lean{HEP} is applied to a type together with a subset of it, whereas for \lean{HEP'} both spaces are subsets of the same ambient type. \lean{HEP'} was the more convenient of the two to work with. Whether the further modification sketched but not implemented of spelling it out with maps defined on the ambient space would be better still, we cannot judge from the experience gathered here. Working in an ambient type is not without cost. Statements become longer since for example \lean{ContinuousOn} obligations are more work to proof than plain \lean{Continuous} ones. \\ 
Both versions of the retraction criterion proved worthwile, and we would recommend establishing both for whichever definition of the HEP one chooses. In \lean{retraction_criterion_closed} the retraction lives on the product with the larger space of the pair, in \lean{retraction_criterion_closed'} on the product with the ambient type. Each was clearly preferable at the place where we used it, as discussed in \cref{subsec:lean:retractioncriterion}. \\

It is the case not just in this project that the effort of formalisation often poorly correlates with its mathematical difficulty. \lean{PartialHomeomorph_HEP'} and its application to obtain \lean{HEP_cube_boundary'} are mathematically unremarkable, yet obtaining the right partial homeomorphism and checking all required properties took considerably long. The proof for two consecutive skeleta consists of a number of individual steps which need to be carried out, none of them surprising. The inductive proof that finite dimensional CW-complex have the HEP was short and caused no difficulties at all, which we take as evidence that the preparatory work was cut to the right shape. \\

In the introduction I already mentioned, that we ran into the case that a statement was claimed slightly different than intended. In \lean{retraction_criterion_closed'} the hypotheses \lean{IsClosed A} asks for closedness in the ambient type not in the larger space of the pair. This strictly stronger hypothesis also occures in \lean{PartialHomeomorph_HEP'}, which uses \lean{retraction_criterion_closed'} in the proof. One could change this assumption to the subset topology, but on the other hand it turned out in practice to rather slightly simplify the application of the theorem than causing any problems. \\

The most obvious point to continue this project would be to generalise the HEP for all relative CW-complexes. For the inductive construction of the retraction maps in \cref{HEP_CW} we would recommend working with retractions on the ambient space, which compose well without any coercions. This would give a further application of \lean{retraction_criterion_closed'}, where we benefit from the formulation using the ambient type. That result could than be used towards the theorems mentioned in the introduction, such as Whitehead's theorem \cref{Whitehead}.A second direction is to formalise the retraction criterion without the closedness assumption, following the argument of \cite{STM} given in the appendix of \cite{HAT01}. By the observation above, this would remove the assumption not only from the criterion itself but from \lean{PartialHomeomorph_HEP'} and everything else proved through it. \\ 
