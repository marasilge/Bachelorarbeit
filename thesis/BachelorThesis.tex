...
The following proposition states, that the HEP behaves transitive for two pairs. In the language of cofibrations, this statement asserts that a composite of cofibrations is again a cofibration, and is stated as in \cite[Problem 5.2]{Strom}
 % (ToDo: Quelle mit Print in der bib abgleichen) https://hbz-ulb.primo.exlibrisgroup.com/discovery/fulldisplay?docid=alma991004996939706467&context=L&vid=49HBZ_ULB:DEFAULT&lang=de&search_scope=MyInst_and_CI&adaptor=Local%20Search%20Engine&tab=Everything&query=any,contains,modern%20classical%20homological%20algebra&offset=0
...
\href{https://github.com/leanprover-community/mathlib4/blob/f30f4b18d1058ce1879f7254ef066ebaed3515f2/Mathlib/Topology/CWComplex/Classical/Basic.lean#L583-L595}{\faExternalLink*}. By this it suffices to consider the intersection of $S$ with the open cells $\openCell{n}{i}$. Since $S \subseteq X_m$ is disjoint from every open cell of dimension greater than $m$, these intersections are empty.
\end{proof}
...
We made use of concepts and conventions of formalising in Lean. I want to briefly explain three of them, which I consider to be the most relevant to this project. 
...
These four abbreviations name the subsets that occur in the definition of the HEP and in the retraction criterion. The set \lean{cyl X} is the cylinder of hight one over the type \lean{X}, whereas \lean{smallcyl A} is the cylinder only over a set \lean{A} of type \lean{Set X}. The set \lean{anchor A} is the union of the cylinder over \lean{(A : Set X)} together with the bottom of the cylinder \lean{cyl X}. Finally, \lean{smallanchor X A} is the corresponding set for the case that \lean{(X A : Set Y)} are subsets of an ambient type \lean{Y}. It  is the union of the cylinder over \lean{A} with the bottom of \lean{smallcyl X}. All four are declared as \lean{abbrev} rather than \lean{def} and are therefore reducible, so that elaboration and \lean{simp} see through them.  
...
def HEP (X : Type u) [TopologicalSpace X] (A : Set X) : Prop :=
  ∀ (Z : Type u) [TopologicalSpace Z] (rangeH' : Set Z), ∀ (f : X → Z), ∀ (H : X × ℝ → Z), Continuous f → Set.range f ⊆ rangeH' → ContinuousOn H (smallcyl A) → (smallcyl A).MapsTo H rangeH' → agreeOn f H A → ∃ H' : X × ℝ → Z, HomotopyExtension H' f H A rangeH'
\end{minted}
...
def HEPY (X : Type u) [TopologicalSpace X] (A : Set X) : Prop :=
  ∀ (Z : Type u) [TopologicalSpace Z], ∀ (f : X → Z), ∀ (H : X × ℝ → Z), Continuous f → ContinuousOn H (smallcyl A) → agreeOn f H A → ∃ (H' : X × ℝ → Z), HomotopyExtensionY H' f H A
\end{minted}
...
Now we can apply the lemma \lean{Topology.RelCWComplex.isClosed_of_disjoint_openCell_or_isClosed_inter_closedCell}, which states that \lean{S'} is closed in the relative CW-complex if its intersection with the base space is closed and if for every cell either \lean{S' ∩ openCell n j} is disjoint or \lean{S' ∩ closedCell n j} is closed. We prove the case of the base space in \lean{hClosedBase} and the case of cells of dimension less than $m$ in \lean{hClosedLT}. For higher dimensions the intersection of \lean{S'} with the open cell disjoint. \\
...
One result deserves to be mentioned separately. The lemma \lean{SkeletonProjectionIsQuotientMap} states that every skeleton of a relative CW-complex is a quotient of the disjoint union of its base space with the closed cubes of the cells it contains. Mathlib describes the topology of a complex through its closed sets rather than defining the complex as a pushout, so this description is not available there. It adds a second point of view on the existing definition, and with it the possibility of constructing a map out of a skeleton by prescribing it on the base space and on each cube separately. This is precisely how the retraction \lean{r} of \Cref{subsubsec:lean:skel:constructionretraction} is built.
 %ToDenken: was genau koennte das sein?

The question that reccured throughout the formalisation was whether to war in an ambient type. It appears in the two definitionally equal versions of the homotopy extension porperty, \lean{HEP} and \lean{HEP'}. \lean{HEP} is applied to a type together with a subset of it, whereas for \lean{HEP'} both spaces are subsets of the same ambient type. Both are used in this project, and \lean{HEP'} was the more convenient of the two to work with. Whether the further modification sketched but not implemented of spelling it out with maps defined on the ambient space would be better still, we cannot judge from the experience gathered here. What we did experience is that working in an ambient type is not without cost. Statements become longer since for example \lean{ContinuousOn} obligations are more work to proof than plain \lean{Continuous} ones. \\ 
