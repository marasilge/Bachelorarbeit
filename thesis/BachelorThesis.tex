...
Throughout the formalisation we use the following abbreviations \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_definition.lean#L40}{\popout}.
...
Already in the mathematical part, the definition of the homotopy extension property was long and technical, and its formalisation is no shorter. We can now turn to it. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_definition.lean#L69}{\popout}
...
This definition of \lean{HEP} is close to the informal \Cref{def:HEP}, apart from three aspects elaborated below. Before turning to them, and as a first check that the definition is usable at all, we prove that every space has the HEP relative to itself and relative to the empty set. The lemma \lean{HEP_self} will serve as the base case of the induction in \Cref{subsec:lean:relCW}. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_definition.lean#L105}{\popout}
\begin{minted}{lean4} 
...
Let us now turn to the variant \lean{HEP'}, which changes the shape of the pair of spaces. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_definition.lean#L96}{\popout}
...
Recall that the retraction criterion was obtained from two lemmas, and the formalisation follows the same structure. Since \Cref{lem:HEPextension} consists of two implications, only one of which requires closedness, we split it into two separate lemmas. The forward direction is called \lean{if_HEP_then_extension}, and the backward direction under the closedness assumption is called \lean{if_extension_then_HEP}. Their proofs are unspectacular and follow the argument in the mathematical section of \Cref{lem:HEPextension} closely. The same holds for the formalisations of \Cref{lem:retract} and of the retraction criterion \Cref{prop:retractioncriterion}. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_definition.lean#L322}{\popout} 
...
The second result is a transitivity property: if the pairs $(A_1,A_2)$ and $(A_2,A_3)$ both satisfy the HEP, then so does the pair $(A_1,A_3)$. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_definition.lean#L509}{\popout} 
\begin{minted}{lean4}
...
The composition of these two homeomorphisms is in particular a partial homeomorphism from \lean{EuclideanSpace ℝ (Fin m)} to \lean{Fin m → ℝ} with source the closed ball and target the closed cube. Applying the lemma \lean{PartialHomeomorph_HEP'} now proves \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_ball_cube.lean#L292}{\popout}
\begin{minted}{lean4}
...
On a cube of top dimension the retraction we need is already available. It is the map \lean{r_cube} that the retraction criterion provides from \lean{HEP_cube_boundary}, the HEP of a cube relative to its boundary proved in \Cref{subsec:lean:ballcube}. On cubes of lower dimension nothing should happen. The map \lean{cubesretract} combines the two cases. Apart from the case distinction it only coerces the second coordinate from the unit interval to \lean{ℝ}. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_skeleton.lean#L160}{\popout}
\begin{minted}{lean4}
...
For \lean{cubesretract} we proved lemmas about its continuity, about which points it fixes, and about the set in which its image lies. The last one comes in a version for the lower dimensions and one for the top dimension, matching the two branches of the definition. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_skeleton.lean#L167}{\popout}
...
Next we assemble these maps into a map on the product of the whole unglued skeleton with the unit interval. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_skeleton.lean#L239}{\popout} 
\begin{minted}{lean4}
def sumMap (m : ℕ) :
...
It remains to push \lean{sumMap} down from the unglued skeleton to the skeleton itself, which is what \lean{Function.extend} does. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_skeleton.lean#L398}{\popout}
...
Altogether this results in the following lemma. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_skeleton.lean#L528}{\popout}
...
We first run the induction over the skeleta, and then show that a finite dimensional complex is one of its own skeleta. In Lean the induction looks as follows. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_relCW.lean#L26}{\popout} 
...
The corresponding statement for \lean{skeleton} then follows from the one for \lean{skeletonLT}, since \lean{skeleton X m} is by definition \lean{skeletonLT X (m+1)}. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_relCW.lean#L37}{\popout} 
\begin{minted}{lean4}
...
It remains to show that for a finite dimensional complex there is an $m \in \mathbb{N}$ with \lean{X = skeletonLT X m}. This is the content of the following lemma. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_relCW.lean#L72}{\popout}
\begin{minted}{lean4}
...
Putting the two steps together, the final proof is short. \href{https://github.com/marasilge/Bachelorarbeit/blob/b918ac6235de35aba40d4a7fa5debcd519f64fa7/Formalisation/HEP_relCW.lean#L85}{\popout}
\begin{minted}{lean4}
