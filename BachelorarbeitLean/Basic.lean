...
lemma zweizweifuenf_rueck {X : Type u} [TopologicalSpace X] {A : Set X} (hA_closed : IsClosed A) :
  ...
  have hg: ContinuousOn g {p | p.2 = 0 ∨ (p.1 ∈ A ∧ p.2 ∈ Set.Icc 0 1)} := by
    ...
    · refine IsClosed.and ?_ ?_
      · have : { x : X × ℝ | x.1 ∈ A} = {x : X | x ∈ A } ×ˢ { a : ℝ | true} := by sorry
        rw[this]
  ...
  ·-- H' MapsTo
    ...
    · simp only [hp, reduceDIte]
      ...
      · simp only [mem_sphere, Subtype.dist_eq, dist_zero_right]
        ...
        · intro h_neg
          simp [h_neg] at hp
      · refine ⟨by sorry, ?_ ⟩ -- ehemals grind
        grw [tsub_le_iff_right, norm_smul_of_nonneg (by sorry) x.1.1, hx.2] -- ehemals grind
...
lemma boundary_hG : frontier (@fun_euclid_max m '' closedBall 0 1) = fun_euclid_max '' (frontier (closedBall 0 1)) := by
  rw [show
      ...
      from rfl]
  rw[closure_hG_cosed, interior_hG, ← Set.image_sdiff bij_euclid_max.injective (closedBall 0 1) (interior (closedBall 0 1)),
...
lemma HEP_cube : ∀ (m : ℕ), m ≥ 0 →
  ...
  apply isHomeomorph_iff_continuous_bijective.2
  constructor
  · fun_prop
  · exact bij_f
